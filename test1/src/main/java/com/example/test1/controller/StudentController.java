...
public class StudentController {
	...
	public String qwer(Model model, @RequestParam HashMap<String, Object> map) throws Exception {
		HashMap<String, Object> resultMap = new HashMap<String, Object>();
		Student stu = studentService.searchQwer();	//Service로 searchQwer() 실행
		resultMap.put("stu", stu); // value는 내가 DB에서 조회한 stu
		resultMap.put("result", "success");
		// {stu : {stuNo : 20153075, stuName : 옥한빛},  result : success }
		return new Gson().toJson(resultMap);
	}
	...
	public String search(Model model, @RequestParam HashMap<String, Object> map) throws Exception {
		//System.out.println(map);	//2개 이상의 데이터를 받을수있게 map
		HashMap<String, Object> resultMap = new HashMap<String, Object>();
		resultMap = studentService.searchStu(map);	//searchStu을 호출하면서 (map)을 넣었다. 
		return new Gson().toJson(resultMap);
	}
